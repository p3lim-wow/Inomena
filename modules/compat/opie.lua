...
local CIRCLE_BORDER = ([[Interface\AddOns\%s\assets\circle]]):format(addon.NAME)
...
addon:HookAddOn('OPie', function()
	OPie.UI:RegisterIndicatorConstructor(addon.NAME, {
		name = addon.NAME,
		apiLevel = 1,
	...
	for _, ring in next, RINGS do
		...
		OneRingLib.ext.RingKeeper:SetRing(id, false) -- delete first
		OneRingLib.ext.RingKeeper:SetRing(id, ring)
	end
end)
